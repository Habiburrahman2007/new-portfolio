...
import { MonitorSmartphone, GitBranch, Plug, TrendingUp, LayoutDashboard } from 'lucide-react';

const CAPABILITIES = [
  {
    title: 'Full-Stack Web Development',
    desc: 'Building scalable and responsive web applications from frontend to backend using modern frameworks and best practices.',
    icon: MonitorSmartphone,
  },
  {
    title: 'Workflow Automation (n8n)',
    desc: 'Designing and implementing automated workflows to connect apps, reduce manual tasks, and improve operational efficiency.',
    icon: GitBranch,
  },
  {
    title: 'API Integration & Development',
    desc: 'Creating and integrating RESTful APIs to enable seamless communication between systems and third-party services.',
    icon: Plug,
  },
  {
    title: 'Business Process Optimization',
    desc: 'Analyzing and transforming manual or inefficient processes into streamlined digital systems using automation and custom tools.',
    icon: TrendingUp,
  },
  {
    title: 'Custom SaaS & Internal Tools',
    desc: 'Developing tailored platforms (dashboards, admin panels, tools) to solve specific business needs and improve productivity.',
    ...
    <section id="capabilities-section" className="pt-24 pb-32 w-full relative bg-[#FAF9F6] overflow-hidden">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 relative z-10">
        ...
        <Gsap.div
          ...
          className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 border-l-2 border-t-2 border-black group/grid bg-[#FAF9F6]"
        >
            ...
            return (
              ...
              >
                ...
                <div className="flex justify-between items-start relative z-10">
                  <span className={`font-mono text-xs md:text-sm font-bold text-black group-hover/cell:text-cyan-400 transition-colors duration-500 tracking-[0.12em] md:tracking-[0.16em] ${isActive ? '!text-cyan-400' : ''}`}>
                    0{i + 1}
                  </span>
                  <cap.icon className={`w-5 h-5 md:w-8 md:h-8 text-black group-hover/cell:text-cyan-400 transition-colors duration-500 ${isActive ? '!text-cyan-400' : ''}`} strokeWidth={2} />
                </div>
                ...
                <div className="relative z-10 mt-auto">
                  <h3 className={`text-base md:text-2xl lg:text-3xl font-black uppercase text-black tracking-tight leading-[1.1] mb-1 md:mb-2 group-hover/cell:text-cyan-400 transition-colors duration-500 ${isActive ? '!text-cyan-400' : ''}`}>
                    {cap.title}
                  </h3>
                  ...
                  <p className={`md:hidden mt-1 text-[11px] border-l-2 border-black group-hover/cell:border-cyan-400 pl-3 text-black/70 group-hover/cell:text-white/80 font-mono leading-5 transition-colors duration-500 ${isActive ? '!border-cyan-400 !text-white/80' : ''}`}>
                    {cap.desc}
                  </p>
                </div>
              </div>
          ...
          {(() => {
            ...
            return (
              <div className={`flex border-r-2 border-b-2 border-black p-5 md:p-8 lg:p-10 min-h-[180px] md:min-h-[300px] lg:min-h-[340px] bg-transparent flex-col justify-center items-center text-center group/ghost hover:bg-[#0A0A0A] transition-colors duration-500 cursor-crosshair ${isGhostActive ? '!bg-[#0A0A0A]' : ''}`}>
                ...
                <span className={`font-mono text-xs tracking-[0.14em] md:tracking-[0.2em] uppercase text-black font-bold group-hover/ghost:text-cyan-400 transition-colors duration-500 ${isGhostActive ? '!text-cyan-400' : ''}`}>
